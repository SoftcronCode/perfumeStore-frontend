...
const CheckoutOption = ({ setSelectedAddress, setSelectedPayment, isCodAvailable }) => {
    ...
    useEffect(() => {
        const fetchAddress = async () => {
            ...
            try {
                const response = await fetchData(API_URLS.getDefaultAddress, requestBody);
                    ...
                    <ul>
                        <li>
                            ...
                            <div className="checkout-box">
                                ...
                                {address.length === 0 ? (
                                    <p>No Address Found! <br /> Please Add New Address or set Default Address. <Link to="/user-dashboard">Click Here.</Link></p>
                                ) : (
                                                    ...
                                                    <div className="delivery-address-box">
                                                        <div>
                                                            <div className="custom-form-check">
                                                                <input className="form-check-input" type="radio" name="address" id="address" onChange={handleAddressChange} value={address.id} />
                                                            </div>
                                                            ...
                                                            <ul className="delivery-address-detail">
                                                                ...
                                                                <li>
                                                                    <p className="text-content"><span className="text-title">Address: </span>
                                                                        {address.house} {address.area}, {address.landmark}, {address.city}, {address.state}, {address.country}</p>
                                                                </li>
                                                                <li>
                                                                    <h6 className="text-content"><span className="text-title">Pin Code:</span> {address.pincode}</h6>
                                                                </li>
                                                                <li>
                                                                    <h6 className="text-content mb-0"><span className="text-title">Phone :</span> {address.phone}</h6>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>
